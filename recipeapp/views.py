...
def review_impl(request):
    ...
    review = ReviewDb().select(int(r_id), (page - 1) * 5);  # 리뷰데이터 가져와서 객체 생성 offset은 페이지당 5개씩 들어가서 (page-1)*5
    reviewpage = (ReviewDb().reviewpage(int(r_id)) + 1);  # 전체페이지수 +1은 초기값이 0이여서 더해둠
